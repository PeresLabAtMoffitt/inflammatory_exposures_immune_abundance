...
clinical_data <- readRDS(paste0(here::here(), "/clinical_data1.rds"))
...
markers_ROI %>% 
  ...
  select(BMI_recent_grp, BMI_YA_grp, diab,
         ...
         pid, education,
         married, 
         anyfhever, anyfhdur, anyfhdur_cat, CD3_total.i) %>% 
...
markers_ROI %>% 
  ...
  select(BMI_recent_grp, BMI_YA_grp, diab,
         ...
         pid, education,
         married, 
         anyfhever, anyfhdur, anyfhdur_cat, CD3_CD8_total.i) %>% 
  ...
  add_overall() %>% bold_labels() %>% add_p() %>% bold_p(t = .05) %>% as_gt()
```
## Intratumoral CD3CD8 total - White
```{r}
markers_ROI %>% 
  filter(race == "White") %>% 
  distinct(suid, .keep_all = TRUE) %>% 
  ...
  add_overall() %>% bold_labels() %>% add_p() %>% bold_p(t = .05) %>% as_gt()
```
## Intratumoral CD3CD8 total - Black
```{r}
markers_ROI %>% 
  filter(race == "Black") %>% 
  distinct(suid, .keep_all = TRUE) %>% 
...
markers_ROI %>% 
  ...
  add_overall() %>% bold_labels() %>% add_p() %>% bold_p(t = .05) %>% as_gt()
```
## Intratumoral Immunoscore - White
```{r}
markers_ROI %>% 
  filter(race == "White") %>% 
  distinct(suid, .keep_all = TRUE) %>% 
  ...
  add_overall() %>% bold_labels() %>% add_p() %>% bold_p(t = .05) %>% as_gt()
```
## Intratumoral Immunoscore - Black
```{r}
markers_ROI %>% 
  filter(race == "Black") %>% 
  distinct(suid, .keep_all = TRUE) %>% 
...
```{r log reg function}
exp_var <- c("BMI_recent_grp", "BMI_recent_5", "BMI_YA_grp", "diab",
             "smokever", "smokcurrent",
                  "packyrs", "packyrs_cat", "talcever", "talcgen", "talcnongen",
                  "aspirin", "NSAID", "aceta", "hyster1yr",
                  "hyster2yr", "hystertype",
...
What are the weight?
```{r}
multinom(immunoscore_2018lancet_patients ~ immunoscore_2018lancet_patients + refage + stage + race,
                     data = markers_ROI)
```
